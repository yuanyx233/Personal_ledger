...
现有记账方式难以在所有设备上统一查看，而 RBC/BMO 暂无符合当前成本与授权约束的可靠自动同步方案。需要一个自用、单用户、当前软件与托管成本为零的 Web/PWA：日常在 iPhone 上即时快速录入，漏记较长时间时再用银行 CSV 补齐，已确认订阅按计划自动生成，并始终汇总到同一份可纠正、可追溯的账本。

## What Changes

- 新建一个以免费 `workers.dev` 地址运行的响应式 Web/PWA，手机、平板和电脑使用同一份数据。
- 新增 iPhone 主屏幕可直达的“记一笔”入口，默认当天、CAD、支出与 `RBC Credit`，正常只要求金额和商户/描述，账户和其他字段可随时修改。
- 新商户在保存后立即显示有界的类别建议和“其他类别”搜索；用户可以选择已有类别，或再次确认后创建新类别，选择同时沉淀为以后可复用的确定性知识规则。
- 新增订阅管理：从历史交易提出候选但必须由用户确认；已确认计划可查看/修改金额、账户、类别、周期、下次扣款日和启用/暂停/取消状态，并在到期日幂等生成实际支出。
- 支持 RBC Visa/chequing 原始 CSV 的安全识别与转换；偶尔补录时，高置信度唯一匹配自动与既有手工/订阅交易合并，歧义项仍由用户确认。
...
- 保留 Cloudflare Access，不开放绕过认证的快捷写入口；在另行获准的远程配置步骤中，把可信个人设备的会话调整为最多一个月以降低重复登录。
...
- `quick-entry`: iPhone 优先的即时录入、默认值、即时类别确认、类别创建与可安装主屏幕入口。
- `subscription-management`: 订阅候选确认、计划生命周期、到期生成、单次未发生处理与银行流水对账。
...
- 新增快速录入默认、订阅计划/发生记录、银行导入合并证据和可迁移导出字段。
